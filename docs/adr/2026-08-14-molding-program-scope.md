...
- **D3 — Versioning: `v5.N.0-gs.1` minors.** M0 tags `v5.1.0-gs.1`; each milestone bumps the minor; the `-gs` suffix is permanent so Go's version selection never auto-upgrades the consumer — every wiremux bump is deliberate. *Amended 2026-08-20: the suffix is retired after `v5.3.0-gs.1`; later releases are plain semver tags. See [the plain-semver tags ADR](2026-08-20-plain-semver-tags.md).*
