...
```sh
go get github.com/the-sarge/turn/v5@v5.3.1
```
...
See the [changelog](CHANGELOG.md) for release notes and the [release runbook](docs/release.md) for publication steps.
...
See the [conventions guide](docs/conventions.md) for observed preferences and the ADRs that define required contracts.

Run the ordinary local gate with `task verify` and the complete exact-head certification with `task preflight`. See [CI configuration ownership](docs/ci-maintenance.md) for shared validation behavior, action-pin equality groups, and inherited configuration status.
